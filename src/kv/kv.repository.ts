...
export class KvRepository {
  ...
  get(path: string): KvEntryRow | null {
    return (
      (this.db
        .prepare(
          `SELECT path, owner_email, nonce_b64, ciphertext_b64, tag_b64, created_at, updated_at
    ...
    this.db
      .prepare(
        `INSERT INTO kv_entries
         (path, owner_email, nonce_b64, ciphertext_b64, tag_b64, created_at, updated_at)
         VALUES (?, ?, ?, ?, ?, ?, ?)
  ...
  delete(path: string): boolean {
    const result = this.db
      .prepare(`DELETE FROM kv_entries WHERE path = ?`)
      .run(path);
